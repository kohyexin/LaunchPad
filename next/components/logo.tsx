  ...
  if (image) {
    return (
      ...
      >
        ...
        <span className="text-white font-bold">Star SaaS</span>
      </Link>
    );
  }
...
export const LogoNavbar = ({ image, locale }: { image?: Image, locale?: string }) => {
  if (image) {
    return (
      <Link
        href={`/${locale || 'en'}`}
...
export const LogoFooter = ({ image, locale }: { image?: Image, locale?: string }) => {
  if (image) {
    return (
      <Link
        href={`/${locale || 'en'}`}
